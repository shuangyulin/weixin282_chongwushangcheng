...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusschongwurizhiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusschongwurizhiCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusschongwurizhi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discusschongwurizhi');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discusschongwurizhi', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusschongwurizhi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusschongwurizhi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discusschongwurizhi', this.detail);
				this.$utils.msg('审核成功');
