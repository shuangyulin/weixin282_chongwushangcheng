					...
					success: async function(res) {
						if (res.confirm) {
							...
							for (let i = 0; i < _this.orderGoods.length; i++) {
                                ...
                                // 当前商品是否相同表
                                if(_this.orderGoods[i].tablename == `chongwuxinxi`){
                                     // 获取数量 
                                     let res = await _this.$api.info('chongwuxinxi', _this.orderGoods[i].goodid);
                                     // 单次限次
						...
						res.data.selected =  res.data.selected  + "," + _this.orderGoods[i].activeSeat
                                         ...
                                         // 库存充足则减少库存数量
                                         await _this.$api.update(`chongwuxinxi`,  res.data);
                                     }
                                }
                                // 当前商品是否相同表
                                if(_this.orderGoods[i].tablename == `chongwuyongpin`){
                                     // 获取数量 
                                     let res = await _this.$api.info('chongwuyongpin', _this.orderGoods[i].goodid);
                                     // 单次限次
                                     if(res.data.onelimittimes>0){
                                        // 超过单次限次购买数量
                                        if(_this.orderGoods[i].buynumber>res.data.onelimittimes){
						...
						res.data.selected =  res.data.selected  + "," + _this.orderGoods[i].activeSeat
                                         ...
                                         // 库存充足则减少库存数量
                                         await _this.$api.update(`chongwuyongpin`,  res.data);
                                     }
                                }
								let order =  {
