<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.shifoujueyu" type="text" placeholder="是否绝育" ></input>
			</view>
        ...
        <view style="display: flex;flex-direction: column;">
          <view :style='categoryName===item.chongwufenlei?sactiveItem:sitem' class="tab" v-for="(item,index) in categoryList" :key="index" :class="categoryName===item.chongwufenlei?'tabActive':''" @tap="categoryClick(item.chongwufenlei)">{{item.chongwufenlei}}</view>
        </view>
      </scroll-view>
        ...
        <view v-if="index%6==0" @tap="onDetailTap(product)" class="list-item" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 0 40rpx 0","borderColor":"red","backgroundColor":"rgba(6, 6, 6, 0.27)","borderRadius":"20rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"320rpx"}'>
            ...
            <text v-if="userid && isAuth('chongwuxinxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('chongwuxinxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('chongwuxinxi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
            <text v-if="!userid && isAuthFront('chongwuxinxi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
...
<script>
	export default {
		...
		async onShow() {
			...
			res.data.list.splice(0,0,{id:0,chongwufenlei:'全部'})
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.chongwufenlei = '%' + this.categoryName + '%'
				}
				...
				if(this.searchForm.shifoujueyu){
			...
			async search(){
				...
				if(this.searchForm.shifoujueyu){
					searchForm['shifoujueyu'] = '%' + this.searchForm.shifoujueyu + '%'
				}
				if(this.searchForm.dianpuming){
