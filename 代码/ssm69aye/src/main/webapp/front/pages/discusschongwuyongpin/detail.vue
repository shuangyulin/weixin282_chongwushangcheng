...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusschongwuyongpinCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusschongwuyongpinCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusschongwuyongpin');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discusschongwuyongpin');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discusschongwuyongpin', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusschongwuyongpin', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusschongwuyongpin/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discusschongwuyongpin', this.detail);
				this.$utils.msg('审核成功');
